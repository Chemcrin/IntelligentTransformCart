...
MecanumResult_t ZDTX42_EnableMotor(uint8_t motor_id, uint8_t enable)
{
    ...
    return Modbus_WriteMultipleRegisters(motor_id, ZDT_REG_ENABLE, regs, 2u, 0u);
}
...
MecanumResult_t ZDTX42_SetMotorSpeedRpm(uint8_t motor_id, float rpm_signed, uint8_t sync_wait)
{
    ...
    (void)sync_wait;
    regs[3] = ZDT_SYNC_NOW_FLAG;

    return Modbus_WriteMultipleRegisters(motor_id, ZDT_REG_SPEED_MODE, regs, 4u, 0u);
}
...
MecanumResult_t ZDTX42_StopMotor(uint8_t motor_id)
{
    ...
    return Modbus_WriteMultipleRegisters(motor_id, ZDT_REG_STOP_IMMEDIATE, &reg, 1u, 0u);
}

MecanumResult_t ZDTX42_StopAllMotors(void)
{
    (void)ZDTX42_SetMotorSpeedRpm(MOTOR_ADDR_FL, 0.0f, 0u);
    (void)ZDTX42_SetMotorSpeedRpm(MOTOR_ADDR_FR, 0.0f, 0u);
    (void)ZDTX42_SetMotorSpeedRpm(MOTOR_ADDR_RL, 0.0f, 0u);
    (void)ZDTX42_SetMotorSpeedRpm(MOTOR_ADDR_RR, 0.0f, 0u);
    return MECANUM_OK;
}
