...
| 接线 | PA2(TX)、PA3(RX) 通过 RS485 模块连接；驱动器端 `R/A/H` 接 RS485 A，`T/B/L` 接 RS485 B；按驱动器实物丝印/手册最终核对 |
...
3. **多个上层任务同时调用 USART2 发送**：必须通过统一的 Modbus 事务状态机串行化。
...
| 状态 | 职责 |
|---|---|
| `INIT` | 初始化 UART1（树莓派通信）、USART2（Modbus）、TIM、GPIO、传感器驱动、Modbus 状态机 |
...
- USART2 由 Modbus 事务状态机独占，不允许传感器调试打印复用 USART2。
...
1. **USART2 RX/TX 中断或 DMA 中断**：保证 Modbus 收发时序准确。
...
- 所有 Modbus 请求通过一个事务队列发送，避免并发访问 USART2。
...
目标：确认 STM32C8T6 通过 USART2 + RS485 能稳定控制单个 ZDT-X42-V2。
...
2. **通信控制**：USART2 使用 PA2/PA3，115200 8N1，RS485 半双工方向由独立 GPIO 控制。
